...
public:
    ...
    SoftieDog(int a, unsigned long data)
    {
        ...
        dispositionBits = (data & ((1UL << 6) - 1));
        data >>= 6;
        pawWebbedBits = (data & ((1UL << 3) - 1));
        data >>= 3;
        pawAreaBits = (data & ((1UL << 7) - 1));
        data >>= 7;
        weightBits = (data & ((1UL << 10) - 1));
        data >>= 10;
        tailAppearance = (data & ((1UL << 2) - 1));
        data >>= 2;
        tailLengthBits = (data & ((1UL << 8) - 1));
        data >>= 8;
        tailColor = (data & ((1UL << 1) - 1));
        data >>= 1;
        pawColor = (data & ((1UL << 1) - 1));
        data >>= 1;
        foregroundColor = (data & ((1UL << 3) - 1));
        data >>= 3;
        foregroundBrightness = (data & ((1UL << 3) - 1));
        data >>= 3;
        backgroundColor = (data & ((1UL << 3) - 1));
        data >>= 3;
        backgroundBrightness = (data & ((1UL << 3) - 1));
        data >>= 3;
        coatSoftnessBits = (data & ((1UL << 6) - 1));
        data >>= 6;
    ...
    unsigned long toUnsignedLong() const
    {
        ...
        data |= coatLengthBits.to_ulong();
        data <<= 6;
        data |= coatSoftnessBits.to_ulong();
        data <<= 3;
        data |= backgroundBrightness.to_ulong();
        data <<= 3;
        data |= backgroundColor.to_ulong();
        data <<= 3;
        data |= foregroundBrightness.to_ulong();
        data <<= 3;
        data |= foregroundColor.to_ulong();
        data <<= 1;
        data |= pawColor.to_ulong();
        data <<= 1;
        data |= tailColor.to_ulong();
        data <<= 8;
        data |= tailLengthBits.to_ulong();
        data <<= 2;
        data |= tailAppearance.to_ulong();
        data <<= 10;
        data |= weightBits.to_ulong();
        data <<= 7;
        data |= pawAreaBits.to_ulong();
        data <<= 3;
        data |= pawWebbedBits.to_ulong();
        data <<= 6;
    ...
    void mutate(int i)
    {
        ...
        while (num_mutations--)
        {
            ...
            if (mutation_point < 8)
            { // Mutate coatLengthBits
                coatLengthBits.flip(mutation_point);
            }
            else if (mutation_point < 14)
            { // Mutate coatSoftnessBits
                coatSoftnessBits.flip(mutation_point - 8);
            }
            else if (mutation_point < 17)
            { // Mutate backgroundBrightness
                backgroundBrightness.flip(mutation_point - 14);
            }
            else if (mutation_point < 20)
            { // Mutate backgroundColor
                backgroundColor.flip(mutation_point - 17);
            }
            else if (mutation_point < 23)
            { // Mutate foregroundBrightness
                foregroundBrightness.flip(mutation_point - 20);
            }
            else if (mutation_point < 26)
            { // Mutate foregroundColor
                foregroundColor.flip(mutation_point - 23);
            }
            else if (mutation_point < 27)
            { // Mutate pawColor
                pawColor.flip(mutation_point - 26);
            }
            else if (mutation_point < 28)
            { // Mutate tailColor
                tailColor.flip(mutation_point - 27);
            }
            else if (mutation_point < 36)
            { // Mutate tailLengthBits
                tailLengthBits.flip(mutation_point - 28);
            }
            else if (mutation_point < 38)
            { // Mutate tailAppearance
                tailAppearance.flip(mutation_point - 36);
            }
            else if (mutation_point < 48)
            { // Mutate weightBits
                weightBits.flip(mutation_point - 38);
            }
            else if (mutation_point < 55)
            { // Mutate pawAreaBits
                pawAreaBits.flip(mutation_point - 48);
            }
            else if (mutation_point < 58)
            { // Mutate pawWebbedBits
                pawWebbedBits.flip(mutation_point - 55);
            }
            else if (mutation_point < 64)
            { // Mutate dispositionBits
...
public:
    ...
    void simulateGeneration(std::vector<SoftieDog> &twoFittestDogs)
    {
        ...
        #pragma omp parallel
        {
            std::vector<SoftieDog> localNewGeneration;
            ...
            for (int i = 0; i < dogs.size() / 2; ++i)
            {
                ...
                for (int j = 0; j < litter_size; ++j)
                {
                    ...
                    localNewGeneration.push_back(puppy); //newGeneration.push_back(puppy);
                }
            }
            #pragma omp critical
            newGeneration.insert(newGeneration.end(), localNewGeneration.begin(), localNewGeneration.end());
        }
        ...
        std::sort(dogs.begin(), dogs.end(), [](const SoftieDog &a, const SoftieDog &b)
                  { return a.softieFitness() > b.softieFitness(); });
        twoFittestDogs.clear();

        if (dogs.size() >= 2)
        {
            twoFittestDogs.push_back(dogs[0]);
